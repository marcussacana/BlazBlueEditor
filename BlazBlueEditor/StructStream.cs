...
namespace AdvancedBinary {
    ...
    class StructWriter : BinaryWriter {
        ...
        public void WriteStruct(ref object Struct) {
            Type type = Struct.GetType();
            WriteStruct(type, ref Struct);
        }

        public void WriteStruct(Type type, ref object Input) {
            FieldInfo[] fields = type.GetFields(BindingFlags.NonPublic | BindingFlags.Instance);
            foreach (FieldInfo field in fields) {
                if (HasAttribute(field, Const.IGNORE))
                    break;
    ...
    class StructReader : BinaryReader {
        ...
        public void ReadStruct(ref object Struct) {
            Type type = Struct.GetType();
            ReadStruct(type, ref Struct);
        }
